...
/* property Image iChannel3: Image { source: "./Shadertoy_Pebbles.png" }
property Image iChannel3: Image { source: "./Shadertoy_Lichen.jpg" }
property Image iChannel3: Image { source: "./Shadertoy_Bayer.png" } */
...
vec3 dither(vec3 color, vec3 bits, vec2 pixel)
{
    ...
    float dithlevel = texture(iChannel3,pixel / iChannelResolution[2].xy).r;
...
vec3 Shade(MarchResult hit, vec3 direction, vec3 camera)
{
    ...
    if(hit.id == SKYDOME)
    {
        ...
        color *= smoothstep(0.5, 0.3, texture(iChannel3, cloudUV, CLOUD_BLUR).r) * 0.5 + 0.5;

        //Mountains
        float a = atan(hit.position.y, hit.position.x)/tau + 0.5;
        a -= 3.28;

        float mountains = MOUNTAIN_SCALE * texture(iChannel3, vec2(a, 0.1),-99.0).r - hit.position.z - MOUNTAIN_SHIFT;
