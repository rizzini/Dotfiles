...
complete -c smbd -s S -l log-stdout -d 'If specified, this parameter causes smbd to log to standard output rather tha…'
complete -c smbd -s i -l interactive -d 'If this parameter is specified it causes the server to run "interactively", n…'
complete -c smbd -s d -l debuglevel -d 'level is an integer from 0 to 10'
complete -c smbd -s V -l version -d 'Prints the program version number'
complete -c smbd -s s -l configfile -d 'The file specified contains the configuration details required by the server'
