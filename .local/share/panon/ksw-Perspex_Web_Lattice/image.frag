...
// property Image iChannel3: Image { source: "./Shadertoy_London.jpg" }
...
float m(vec3 p){
    ...
    float h = heightMap(p); // texture(iChannel3, p.xy/2.).x; // Texture work too.
...
vec3 eMap(vec3 rd, vec3 sn){
    ...
    //vec3 tx = tex3D(iChannel3, rd/3., sn);
...
void mainImage(out vec4 c, vec2 u){
    ...
    if(t<FAR){
        ...
        // texture a bit.
        vec3 tx = tex3D(iChannel3, (p*2. + hm*.2), n);
