...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    bmask *= 1.-texture(iChannel3, fragCoord.xy/iChannelResolution[0].xy).r*0.4*pow((0.9-bmask*0.9), 2.);

    fragColor.rgb = bmask*LASERCOL;

    float g = gridAA(uv);
    g *= 1.-texture(iChannel3, fragCoord.xy/iChannelResolution[0].xy).r*0.1*pow((0.9-bmask*0.9), 2.);
