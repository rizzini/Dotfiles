...
vec3 noised( in vec2 x )
{
    ...
    float a = texelFetch( iChannel3, (p+ivec2(0,0))&255, 0 ).x;
	float b = texelFetch( iChannel3, (p+ivec2(1,0))&255, 0 ).x;
	float c = texelFetch( iChannel3, (p+ivec2(0,1))&255, 0 ).x;
	float d = texelFetch( iChannel3, (p+ivec2(1,1))&255, 0 ).x;
#else
    // texture version
    vec2 p = floor(x);
	float a = textureLod( iChannel3, (p+vec2(0.5,0.5))/256.0, 0.0 ).x;
	float b = textureLod( iChannel3, (p+vec2(1.5,0.5))/256.0, 0.0 ).x;
	float c = textureLod( iChannel3, (p+vec2(0.5,1.5))/256.0, 0.0 ).x;
	float d = textureLod( iChannel3, (p+vec2(1.5,1.5))/256.0, 0.0 ).x;
#endif
...
vec3 render( in vec3 ro, in vec3 rd )
{
	...
	if( TEST_MAT_LESS (m, MAT_TERRAIN_TRAIL ) )
	{
	    ...
	    float textureGlitter  = textureLod(iChannel3,pos.xz * _TerrainGlitterRep, 2.2).x * 1.15;
