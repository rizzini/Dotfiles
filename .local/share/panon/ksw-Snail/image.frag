...
float mapShell( in vec3 p, out vec4 matInfo )
{
    ...
    float di = textureLod( iChannel3, vec2(t+r,0.5), 0. ).x;
    d += 0.002*di;
...
vec2 mapSnail( vec3 p, out vec4 matInfo )
{
    ...
    // disp
    float dis = textureLod( iChannel3, 5.0*p.xy, 0. ).x;
...
vec3 shadeOpaque( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo )
{
    ...
    if( m<1.5 ) // snail body
    {
        float dis = texture( iChannel3, 5.0*pos.xy ).x;
        ...
        f = 1.0-pow( f, 8.0 );
        f = 1.0 - (1.0-f)*(1.0-texture( iChannel3, 0.3*pos.xy ).x);
    ...
    else if( m<2.5 ) // shell
    {
        ...
        vec3 ral = texture( iChannel3, vec2(2.0*matInfo.w+matInfo.z*0.5,0.5) ).xxx;
        mateD *= 0.25 + 0.75*ral;
    ...
    else if( m<3.5 ) // plant
    {
        ...
        vec3 te = texture( iChannel3, pos.xy*0.2 ).xyz;
        mateS *= 0.5 + 1.5*te;
    ...
    else //if( m<4.5 ) // leave
    {
        ...
        vec3 te = texture( iChannel3, p.xz*0.35 ).xyz;
...
vec3 shadeTransparent( in vec3 ro, in vec3 rd, in float t, in float m, in vec4 matInfo, in vec3 col, in float depth )
{
    ...
    col += vec3(0.8,1.0,0.8)*0.5*smoothstep(0.3,0.6,texture( iChannel3, 0.8*nor.xy ).x)*(0.1+0.9*fre*fre);
...
vec3 background( in vec3 d )
{
    ...
    for( int i=ZERO; i<200; i++ )
    {
        ...
        vec3 tmp = texture( iChannel3, uv*0.25 + 0.0075*of, 4.0 ).yxz;
