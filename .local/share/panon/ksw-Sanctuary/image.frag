...
// property Image iChannel3: Image { source: "qrc:/Qml/Shader/4ddfwx.png" }
...
vec4 trip(sampler2D sampler, vec3 nml, vec3 pos)
{
    ...
    b = pow(b, vec3(50.));
    b /= dot(b, vec3(1.));
    return texture(iChannel3, pos.yz) * b.x + texture(iChannel3, pos.xz) * b.y + texture(iChannel3, pos.xy) * b.z;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    {
        float k = .001;
        diffMap = trip(iChannel3, nml, hit * 1.111).xyz;
        vec3 diffMap2 = trip(iChannel3, nml, hit * 1.111 + vec3(.005)).xyz;
    ...
    else
    {
        ...
        float k = .001;
        diffMap = trip(iChannel3, nml, mov * 1.111).xyz;
        vec3 diffMap2 = trip(iChannel3, nml, mov * 1.111 + vec3(.005)).xyz;
        nml = normalize(nml + (diffMap.x - diffMap2.x) * .7);
    }
