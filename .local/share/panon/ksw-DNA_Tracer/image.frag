...
// property Image iChannel3: Image { source: "./Shader_DNA_Tracer_iChannel3.png" }
...
vec4 texNoise(vec2 uv){ float f = 0.; f+=texture(iChannel3, uv*.125).r*.5; f+=texture(iChannel3,uv*.25).r*.25; //MERCURTY SDF LIBRARY IS HERE OFF COURSE: http://mercury.sexy/hg_sdf/
    f+=texture(iChannel3,uv*.5).r*.125; f+=texture(iChannel3,uv*1.).r*.125; f=pow(f,1.2);return vec4(f*.45+.05);}
