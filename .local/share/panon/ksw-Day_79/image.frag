...
// property Image iChannel3: Image { source: "./Shader_Day_79_iChannel3.png" }
...
vec4 valueNoise(float t){
    return mix(texture(iChannel3, vec2(t)/256.),texture(iChannel3, vec2(t+1.)/256.),smoothstep(0.,1.,fract(t)));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    N = texture(iChannel3,(uv)*256.).x*1.;
    uv *= 1. + dot(uv,uv)*0.25;
    vec3 col = vec3(0);
