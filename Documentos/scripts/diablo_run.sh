...
if [ "$1" == 'BR' ]; then
    WINEDEBUG=-all WINEFSYNC=1 WINEESYNC=1 WINEFSYNC_FUTEX2=1 WINEPREFIX=/home/lucas/.wine_d3/ /usr/bin/gamemoderun /usr/bin/wine_tkg /mnt/archlinux/Games/DIABLO_III/Diablo\ III.exe -launch &> /dev/null &
elif [ "$1" == 'US' ]; then 
     WINEDEBUG=-all WINEFSYNC=1 WINEESYNC=1 WINEFSYNC_FUTEX2=1 WINEPREFIX=/home/lucas/.wine_d3/ /usr/bin/gamemoderun /usr/bin/wine_tkg /mnt/archlinux/Games/DIABLO_III/Diablo\ III.exe -launch OnlineService.Matchmaking.ServerPool=Default &> /dev/null &
fi
