...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    if(t<FAR){
        ...
        float sz = 1./3.;
        n = db(iChannel3, p*sz, n, .1/(1. + t*.25/FAR));
        ...
        vec3 tx = vec3(.05); // tpl(iChannel3, p*sz, n);
        ...
        vec3 refl = texture(iChannel3, normalize(reflect(r, svn*.5 + n*.5))).xyz;
        vec3 refr = texture(iChannel3, normalize(refract(r, svn*.5 + n*.5, 1./1.31))).xyz;
        refl *= refl*.5;
        refr *= refr*.5;
        */
