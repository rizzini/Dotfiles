...
//	property Image iChannel3: Image { source: "./Shader_Circuits_iChannel3.jpg" }
...
void formula(vec2 z, float c) {
	...
	vec3 col=normalize(.1+texture(iChannel3,vec2(minit*.1)).rgb);
	color+=col*(.4+mod(minit/9.-time*10.+ot2*2.,1.)*1.6);
	color+=vec3(1.,.7,.3)*circ*(10.-minit)*3.*smoothstep(0.,.5,.15+texture(iChannel3,vec2(.0,1.)).x-.5);
}
