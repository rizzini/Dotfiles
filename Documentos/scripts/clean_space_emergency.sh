...
while ;do
    {
    if $(/usr/bin/df -B MB  /dev/sda2 --output=avail | /usr/bin/tail -1 | /usr/bin/tr -d 'MB') -ge 700 ];do
    ...
    if $(/usr/bin/df -B MB  /dev/sdb2 --output=avail | /usr/bin/tail -1 | /usr/bin/tr -d 'MB') -ge 700 ];do
